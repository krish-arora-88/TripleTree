...
const inputWrap = $("#input-canvas-wrap");
const inputCanvas = $("#input-canvas");
const seedMarker = $("#seed-marker");
const algoSelect = $("#algo-select");
const tolSlider = $("#tolerance-slider");
const tolValue = $("#tolerance-value");
const pickerSelect = $("#picker-select");
const pickerParams = $("#picker-params");
const frameFreqInput = $("#frame-freq-input");
const maxFramesInput = $("#max-frames-input");
const runBtn = $("#run-btn");
const warningBanner = $("#warning-banner");
const statusEl = $("#status");
const outputWrap = $("#output-canvas-wrap");
const outputCanvas = $("#output-canvas");
const outputPlaceholder = $("#output-placeholder");
const frameScrubber = $("#frame-scrubber");
const frameSlider = $("#frame-slider");
const frameIndex = $("#frame-index");
const frameTotal = $("#frame-total");
const exportBtns = $("#export-buttons");
const downloadPng = $("#download-png");
const uploadAnother = $("#upload-another");
const seedSizeSlider = $("#seed-size-slider");
const seedSizeValue = $("#seed-size-value");
// ---- Worker ----------------------------------------------------------------
let worker = null;
function getWorker() {
    if (!worker) {
...
function getPickerParamsArray() {
    const picker = parseInt(pickerSelect.value, 10);
    switch (picker) {
        case 0: {
            const c = hexToRGBA(($("#pp-color")?.value) ?? "#ff0000");
            return [...c];
        }
        case 1: {
            const c1 = hexToRGBA($("#pp-color1")?.value ?? "#ff8000");
            const c2 = hexToRGBA($("#pp-color2")?.value ?? "#0080ff");
            const sw = parseInt($("#pp-stripe-width")?.value ?? "10", 10);
            return [...c1, ...c2, sw];
        }
        case 2: {
            const c = hexToRGBA($("#pp-color")?.value ?? "#ff0000");
            const bright = parseInt($("#pp-bright")?.value ?? "40", 10);
            return [...c, bright, 0, 0];
        }
        case 3: {
            const fc = hexToRGBA($("#pp-fill-color")?.value ?? "#00ff00");
            const bc = hexToRGBA($("#pp-border-color")?.value ?? "#ff0000");
            const bw = parseInt($("#pp-border-width")?.value ?? "3", 10);
            return [...fc, ...bc, bw];
        }
        default:
            return [255, 0, 0, 255];
...
function validateParams() {
    ...
    if (isNaN(tol) || tol < 0 || tol > 2)
        return "Tolerance must be between 0 and 2.";
    const freq = parseInt(frameFreqInput.value, 10);
    if (isNaN(freq) || freq < 0)
        return "Frame frequency must be >= 0.";
    const maxF = parseInt(maxFramesInput.value, 10);
    if (isNaN(maxF) || maxF < 0)
        return "Max frames must be >= 0.";
    return null;
}
function runFill() {
    if (!state.imageData || !state.seedSet || state.running)
        return;
...
function showFrame(width, height, index) {
